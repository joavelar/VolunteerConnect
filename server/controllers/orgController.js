  ...
  checkEmailExistsInVolunteers(email)
    .then((emailExistsInVols) => {
      if (emailExistsInVols) {
        return res.status(400).json({ message: 'This email is already associated with a volunteer account.' });
      }
    ...
    .catch((err) => {
      res.status(500).json({ message: 'Email check failed', error: err.message });
    });
};
